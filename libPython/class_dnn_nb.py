...
class DNN_denseGruDense_NB(pl.LightningModule):
    """Narrowband/Hybrid DNN for step-size inference."""

    def __init__(self, dnn_dic):

        super(DNN_denseGruDense_NB, self).__init__()
        ...
        self.dec_mask_mu_mod_list = torch.nn.ModuleList()
        self.dec_mask_mu_mod_list.append(torch.nn.Linear(self.hidden_size_gru, 1))
        self.dec_mask_mu_mod_list.append(torch.nn.Sigmoid())

        # decoder architecture for estimation of mask Ef
        if self.est_mask_Ef == 'dnnFreq':
            self.dec_mask_Ef_mod_list = torch.nn.ModuleList()
            self.dec_mask_Ef_mod_list.append(torch.nn.Linear(self.hidden_size_gru, 1))      # self.dec_mask_Ef_mod_list.append(torch.nn.Linear(self.hidden_size_gru, 1))
            self.dec_mask_Ef_mod_list.append(torch.nn.Sigmoid())

        elif self.est_mask_Ef == 'dnnScal':
            raise Exception('This is not implemented. Use broadband DNN.')
    ...
    def forward_block(self, sig_block_dic, gru_state_in):
        ...
        # reshape to concatenate batch and frequency dim
        feat_vec = torch.flatten(feat_maps, start_dim=2, end_dim=-1)        # flatten feature dimension
        feat_vec = torch.flatten(feat_vec, start_dim=0, end_dim=1)          # flatten frequency and batch dimension
        ...
        dnn_masks_aec_dic = self.forward_block_decoder(x_out_gru, feat_maps.shape[0])

        return dnn_masks_aec_dic, gru_state_out

    def forward_block_decoder(self, x_out_gru, num_batch):
        """Inference of step-size masks from GRU states."""

        # decoder inference of mu mask
        x = x_out_gru
        for dec_block in self.dec_mask_mu_mod_list:
            x = dec_block(x)
        mask_mu = torch.reshape(x, [num_batch, self.mask_freq_size, 1])

        # decoder inference of Ef mask
        if self.est_mask_Ef == 'dnnFreq':
            x = x_out_gru
            for dec_block in self.dec_mask_Ef_mod_list:
                x = dec_block(x)
            mask_Ef = torch.reshape(x, [num_batch, self.mask_freq_size])
    ...
    def init_hidden(self, batch_size):
        ...
        init_state = torch.zeros((self.num_layers_gru, batch_size * self.mask_freq_size, self.hidden_size_gru), requires_grad=False, device=self.device)
